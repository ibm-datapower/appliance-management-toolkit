...
/**
 * An operation was prematurely terminated because the Manager is being shut
 * down. In the case of a BackgroundTask or Notification, those Tasks will be
 * discarded and not executed. The reason why we need to mark Tasks as
 * "terminated because of shutdown" is because there typically are
 * ProgressContainers associated with these Tasks, and it is possible that
 * another thread may be blocking waiting for those ProgressContainers to
 * complete. Without marking the ProgressContainers complete via this exception,
 * it is possible that the other threads may wait forever on a ProgressContainer
 * that will never be completed. So it is likely the only place you will see
 * this exception used is inside a ProgressContainer.
 * <p>
 * @version SCM ID: $Id: ShutdownException.java,v 1.2 2010/08/23 21:20:27 burket Exp $
 * <p>
 */
public class ShutdownException extends ClientAPIException {
     ...
     *        msgKey
     */
    public ShutdownException(String message, String msgKey,
            Object[] msgParms) {
